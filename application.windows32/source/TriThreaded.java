...
public class TriThreaded extends PApplet {
...
public void draw() {
  ...
  if (testing) {
    graphics.stroke(120);
    graphics.line(0, height/2.0f, 0, width, height/2.0f, 0);
    graphics.line(width/2.0f, 0, 0, width/2.0f, height, 0);
  }
  ...
  public void run() {
    ...
    while (true) {
      ...
      for (int i = 1; i < used_in + 1; i++) {
        ...
        float  band = max(0, (rband+lband)/2.0f);
      ...
      avg /= navg;  
      pmax = max(pmax-(gmax-pmax)/3.0f, pmax - decay/3.0f);
      gmax = max;

      if(last_click > 2000){
        if (avg > 30 ||gmax > 60) {
          pattern = 0;
        } else if (gmax < 50 && t%next_seq <= 0.03f) {
          pattern = floor(random((num_patt + 1)))%num_patt;
          next_seq = ((int)random(200) + 1)*0.02f;
          if (testing) {
...
public void spectrum() {
  int w = 1; 
  float s_pos = height/2.5f;
  
  spec_running+=gmax/10000.0f;
  for (int i = 0; i < used_in-w; i+=w) {
    float s = spec_x;
    float s2 = 5;
    if(i*s > 2*width/5.0f) break;
    //top left
    if(gmax > 65){
      ...
      float alph = 255*sin(spec_running)-150*sin(i*0.005f);
      if(alph <= 0) break; 
      ...
      float alph = 25+100*sin(PI*t)-100*sin(i*0.005f);
      if(alph <= 0) break; 
    ...
    if(y12 > 100){ 
      y12 = min( y12*.5f, height/6.0f);
    }
    float y2 = plevels[i+w]*s2;
    if(y2 > 100){
      y2 = min( y2*.5f, height/6.0f);
    }
    //left
    graphics.beginShape();
...
public void topSpec() {
  ...
  for (int i = 0; i < TS_n-1; i++) {
    ...
    TS_mag_mix[i+1] = TS_mag_mix[i+1]/2.75f;
    TS_mag2[i+1] = TS_mag2[i+1]/2.75f;
    TS_mag[i+1] = TS_mag[i+1]/2.75f;

    float tr = 255*sin(i*0.5f) - random(5)*i;//255-random(2)*i
    float tb = random(255);
    float tg = random(255);

    float br = 255*sin(PI-i*0.25f) + random(5)*i;//255-random(5)*i
    float bb = random(255);
    float bg = random(255);
    ...
    if (gmax > 25 && i < TS_n*1/2) {  
      aurora(i, 255.0f*sin(i*.05f), 100.0f, 0.0f, floor(200*sin(10*PI*t/gmax)));
    }
    
    if (gmax > 35 && i < TS_n/4  && (t%4 > 3)) {
      aurora_spread(i, 255.0f*sin(t*10), 255*sin((random(1)-1)*-t*10), 255.0f*cos(PI*t/7), 100*sin(i*0.2f)+100.0f, floor(gmax/20));
    }
...
public void aurora(int i, float tr, float tb, float tg, float ta) {
  float s = spec_x;
  float s2 = 7;
  float max_y = height/7.0f;
  float y_pos = height/2.5f;
...
public void aurora_spread(int ind, float tr, float tb, float tg, float ta, int reps) {
  float s = spec_x*gmax/100.0f;
  float s2 = 7;
  ...
  graphics.stroke(tr, tb, tg, ta);
  graphics.fill(tr*4.0f/5.0f,tb,tg,ta*2.0f/3.0f);
  //left
  for (int i = 0; i < reps; i++) {
    graphics.pushMatrix();
    graphics.translate(lx1, height/3.0f-10.0f*sin(lx1+20*t), 0);
    graphics.rotateZ((90*i+20*sin(2*t)) * ly2/ly1);
    graphics.beginShape(TRIANGLES);
    graphics.vertex(0, 0, 0);
    graphics.vertex(0, ly1*((i+1)*3/reps), 0.0f);
  ...
  for (int i = 0; i < reps; i++) {
    graphics.pushMatrix();
    graphics.translate(width-rx1, height/3.0f-10.0f*sin(rx1+20*t), 0);
    graphics.rotateZ((90*i+20*sin(2*t)) * ry2/ry1);
    graphics.beginShape(TRIANGLES);
    graphics.vertex(0, 0, 0);
    graphics.vertex(0, ry1*((i + 1)*3/reps), 0.0f);
...
public void fourLine(int i, float tr, float tb, float tg, float ta, float br, float bb, float bg, float ba) {
  float s = spec_x;
  float s2 = 20;
  float max_y = 200;
...
public void all_rings() {
  float s = sin(t);

  graphics.stroke(0);  
  graphics.fill((int) random(255), 222, random(255), 100);
  ring(width/2.0f, height/2.0f, 3, ring_radius[0], 2*t, false);
  graphics.fill(31, 182, 222, 100);
  ring(width/2.0f, height/2.0f, 3, ring_radius[1], 2*t, true);
  graphics.fill(200, 180, 0, 100);
  ring(width/2.0f, height/2.0f, 3, ring_radius[2], PI+2*t, true);
  graphics.fill(222, 31, 31, 100);
  ring(width/2.0f, height/2.0f, 3, ring_radius[3], 2*t, true);
  graphics.fill(218, 222, 31, 150);
  ring(width/2.0f, height/2.0f, 6, ring_radius[4], t, true);
  graphics.fill(229, 35, 35, 100);
  ring(width/2.0f, height/2.0f, 11, 125, t, true);
  graphics.fill(229, 136, 35, 100);
  ring(width/2.0f, height/2.0f, 10, 125, -t, true);
  graphics.fill(239, 255, 67, 100);
  ring(width/2.0f, height/2.0f, 12, 125, 1.5f*t, true);
  graphics.fill(255, 255, 255, 10);
  ring(width/2.0f, height/2.0f, 6, ring_radius[5], t, true);
  ring(width/2.0f, height/2.0f, 6, ring_radius[5] + 2, 1.1f*t, true);
  ring(width/2.0f, height/2.0f, 6, ring_radius[5] + 4, 1.2f*t, true);
  ring(width/2.0f, height/2.0f, 6, ring_radius[5] + 6, 1.3f*t, true);
  graphics.fill(0, 0, 0, 0);
  graphics.stroke(255);
  equalizerRing(width/2.0f, height/2.0f, num_bars, t);
  ring(width/2.0f, height/2.0f, 100, 177.5f, -.75f*t+2*s, false);
  ring(width/2.0f, height/2.0f, 100, 175, -.75f*t+2*s + PI/100, true);
}
...
public void backgroundPattern() {
  //scale edited to 10x10 at screen dimensions to 15x15 at 1.5x dimensions to reduce edge clipping problems during large volume sweeps
    int hn = 15;
    float hx = 1.5f*height/hn;
    int wn = 15;
    float wx = 1.5f*width/wn;
    int q  = 1;
    float s = sin(t);
    graphics.pushMatrix();
    graphics.translate(0, 0, 50-.1f*max((200-7*s+gmax), (200-7*s+pmax)));
    for (float i = -hx - frameCount%(2*hx); i < (hn + 1)*hx; i += hx) {
      ...
      for (int j = 0; j < wn; j++) {
        ...
        graphics.stroke(sin(7.0f*gmax*i)*255, sin(t)*222, 77*sin(t)+55, random(100, 200));
...
public void tri(float _x, float _y, float _z, float _r, float _s, boolean ori) {
  ...
  if (ori) {
    graphics.rotateZ(PI/2.0f-_r);
  } else {
    graphics.rotateZ(PI+PI/2.0f-_r);
  }
